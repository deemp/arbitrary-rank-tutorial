...
type Name = Text -- Names are very simple
...
data TyVar
  = BoundTv Text -- A type variable bound by a ForAll
...
instance Eq TyVar where
  ...
  _ == _ = error "Mismatched type variables"
...
freeTyVars tys = foldr (go []) [] tys
 where
  go ::
    [TyVar] -> -- Ignore occurrences of bound type variables
...
instance Outputable Term where
  ...
  ppr (Lam v e) = sep [pretty '\\' <> pprName v <> pretty ".", ppr e]
  ppr (ALam v t e) =
    sep
      [ pretty '\\'
