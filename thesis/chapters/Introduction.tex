...
\begin{itemize}
    \item \textbf{\Cref{chap:LiteratureReview}} reviews the foundational paper on arbitrary-rank type inference and discusses the key architectural concepts from GHC that inform this work.
    \item \textbf{\Cref{chap:DesignImplementation}} details the design and implementation of the Arralac language, covering its pipeline from parsing to typechecking and evaluation, with a focus on the architectural decisions made.
    \item \textbf{\Cref{chap:EvaluationDiscussion}} evaluates the resulting system against its pedagogical goals and discusses the insights gained during the development process.
