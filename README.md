...
- After typechecking, Haskell programs are to desugared to Core [^WITS2024], a small language based on System FC [^GhcSystemFcOutdated][^GhcSystemFcCurrent] that is relatively easy to typecheck and manipulate.

- The `Arralac` implementation also uses a Core language based on untyped lambda calculus [^ATutorialIntroductionToTheLambdaCalculus]. It only has lambda abstraction, application, `let`-bindings, and literals.

- GHC implements capture-avoding substitution based on the `Rapier` approach [^GhcRapier] [^GhcSubstitutingBinders] [^UniqAway].

- [`Core.Scoped`](./arralac/src/Language/Arralac/Core/Scoped.hs) provides the `SCore` type that has scoped lambda and `let`-bindings. This type is constructed via the `free-foil`  library [^FreeFoilLibrary]. The `Free Foil` approach [^FreeFoil] makes capture-avoiding substitution type-safe and efficient. It is a significant improvement in terms of type safety over the `Foil` approach [^Foil], which is an attempt to make the untyped `Rapier` approach type-safe.
...
[^GhcRapier]: <https://www.microsoft.com/en-us/research/wp-content/uploads/2002/07/inline.pdf>

[^GhcSubstitutingBinders]: <https://github.com/ghc/ghc/blob/ed38c09bd89307a7d3f219e1965a0d9743d0ca73/compiler/GHC/Core/Subst.hs#L332>

[^UniqAway]: <https://github.com/ghc/ghc/blob/ed38c09bd89307a7d3f219e1965a0d9743d0ca73/compiler/GHC/Types/Var/Env.hs#L214>

[^Foil]: <https://doi.org/10.1145/3587216.3587224>

[^FreeFoil]: <https://github.com/fizruk/free-foil>

[^LspHaskell]: <https://hackage.haskell.org/package/lsp>

[^OptparseApplicative]: <https://hackage.haskell.org/package/optparse-applicative>
